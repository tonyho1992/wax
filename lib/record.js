            ...
            for (var i = 0; i < obj.length; i++) {
                if (itr = obj[i]['@c']) {
                    for (var j = 0; j < itr.length; j++) {
                        var args = [];
                        if (itr[j][0] && (fn = this.get_function(itr[j][0], cur))) {
                            if (itr[j][1].length) {
                                args = args.concat(wax.Wax.reify(itr[j][1]));
                            }
                            if (cur) {
                                cur = fn[1].apply(cur, args);
                            } else {
                                cur = fn[1].apply(fn[0], args);
                            }
                        } else {
                            if (fn = this.get_function(itr[j]['@n'][0])) {
                                var args = _.map(itr[j]['@n'][1], function(a) {
